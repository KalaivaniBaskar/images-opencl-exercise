    ...
    */
    ...
    try 
    {
     ...
     // Get the command queue
        ...
        //  cl::Program program(context, util::loadProgram("vsum.cl"), true);
    ...
    //   /* for checking kernel errors 
        ...
        const char* kernelSource = R"(
            ...
            __kernel void vsum(__global const int* input, __global int* output, const int size) {
                __local int localSum[WORK_GROUP_SIZE];
                ...
                while (globalIndex < size) {
                    localSum[localID] += input[globalIndex];
                ...
                for (int stride = groupSize / 2; stride > 0; stride /= 2) {
                    if (localIndex < stride) {
                        localSum[localIndex] += localSum[localIndex + stride];
                    }
                    barrier(CLK_LOCAL_MEM_FENCE);
                }
...
//*/
        ...
        auto vsum = cl::make_kernel<cl::Buffer, cl::Buffer, int>(program, "vsum");
      ...
      //  d_b = cl::Buffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,  sizeof(int) * size*size*size*size, h_b.data());
        ...
        std::cout<<"Total Vector Sum: "<< totalsum << std::endl;
