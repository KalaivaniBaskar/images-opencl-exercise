...
{   
    ...
    const int pM = 2;
    const int pN = 2;
    int opM = ((M - pM)/ stride ) + 1;
    int opN = ((N - pN)/ stride ) + 1;
    ...
    try 
    {
     ...
     // Get the command queue
        ...
        //  cl::Program program(context, util::loadProgram("max_pool.cl"), true);

    //   /* for checking kernel errors 

        const char* kernelSource = R"(
            __kernel void max_pool(                             
            __global int* input,                      
            __global int* filter,                      
            __global int* output,                      
            const  int M,
            ...
            if( gidI < opM && gidJ < opN) {
               int cols = gidI * stride + pN;
               int rows = gidJ * stride + pM; 
               int count = 0;
               ...
               int max = INT_MIN;
               for(int k=0; k < (pM * pN); k++ ){
                 // printf("inp %d and filt %d \n", inp[k], filter[k]);
                  if(inp[k] > max)
                     max = inp[k];
               } 
               //printf("for i %d, j %d , max %d\n", gidI , gidJ, sum);
               output[gidI * opN + gidJ] = max;
...
//  */
        ...
        auto max_pool = cl::make_kernel<cl::Buffer, cl::Buffer, cl::Buffer, int, int, int, int, int>(program, "max_pool");
