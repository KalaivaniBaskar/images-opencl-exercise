...
{   
    ...
    std::array<std::array<std::array<float, opO>, opN>, opM> output;

    std::vector<int> h_input(M * N * O);    // input
    
    std::vector<float> h_output(opM * opN * opO);    // convolution output w stride =1, no padding 
        ...
        {                                          
            ...
            float sum = 0.0f; 
              //printf("for gid %d %d %d the limits are %d %d %d \n", gidI, gidJ, gidK, d, h, w);
                    ...
                    for (int n = gidJ * stride - padding; n < h; ++n) {
                        for (int k = gidK * stride - padding; k < w; ++k) {
                            ...
                            if (m >= 0 && m < M && n >= 0 && n < N && k >= 0 && k < O) {
                                inp[count] = (float) input[(m * (N * O) + n * O + k)];                              
                ...
                output[gidI * (opN * opO) + gidJ * (opO) + gidK] = sum / W_SIZE;
...
//  */
       ...
       // d_a   = cl::Buffer(context, begin(h_a), end(h_a), true); 
        ...
        d_c  = cl::Buffer(context, CL_MEM_WRITE_ONLY, sizeof(float) * (opM * opN * opO) );
